...
// 载入PNG图并去透明部分
void  Tool::putimagePNG(int  picture_x, int picture_y, IMAGE* picture) //x为载入图片的X坐标，y为Y坐标
{
...
// 适用于 y <0 以及x<0的任何情况
void Tool::putimagePNG2(int x, int y, IMAGE* picture) {
    IMAGE imgTmp;
    if (y < 0) {
...
// 适用于 y <0 以及y>0的任何情况
void  Tool:: putimagePNG2(int x, int y, int winWidth, IMAGE* picture) {
    IMAGE imgTmp;
    if (y < 0) {
        SetWorkingImage(picture);
        getimage(&imgTmp, 0, -y,
...
bool  Tool:: rectIntersect(int x01, int y01, int x02, int y02,
...
void  Tool::preLoadSound(const char* name) {
    char cmd[512];
...
void  Tool::playSound(const char* name) {
    static int index = 1;
    char cmd[512];
...
void  Tool:: drawBloodBar(int x, int y, int width, int height, int lineWidth, int boardColor, int emptyColor, int fillColor, float percent) {
    LINESTYLE lineStyle;
...
void Tool::Button(int x, int y, int width, int height, const char* str, COLORREF color)
{
    setcolor(color);          //设置按钮颜色
    setfillcolor(color);           //设置按钮填充颜色
    fillrectangle(x, y, x + width, y + height);
    setcolor(RGB(37,26,89));  //设置字体颜色
    setbkmode(TRANSPARENT);
    outtextxy(x + (width - textwidth(str)) / 2, y + (height - textheight(str)) / 2, str);
